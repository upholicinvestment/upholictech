...
interface FIIData {
  Date: string;
  Pro_Call_Change: string;
  Pro_Put_Change: string;
  NIFTY_Value: string;
}
...
const Pro_Index_Opt: React.FC = () => {
  ...
  useEffect(() => {
    axios
      .get<FIIData[]>("http://localhost:5000/api/Pro_Index_Opt/data")
      .then((response) => {
        const formattedData = response.data.map((item) => {
          ...
          return {
            date: item.Date,
            callChange: parseValue(item.Pro_Call_Change),
            putChange: parseValue(item.Pro_Put_Change),
