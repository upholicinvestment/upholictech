...
interface FIIData {
  Date: string;
  FII_Call_Change: string;
  FII_Put_Change: string;
  NIFTY_Value: string;
}
...
const Fii_Dii_Fno: React.FC = () => {
  ...
  useEffect(() => {
    axios
      .get<FIIData[]>("http://localhost:5000/api/data")
      .then((response) => {
        const formattedData = response.data.map((item) => {
          ...
          return {
            date: item.Date,
            callChange: parseValue(item.FII_Call_Change),
            putChange: parseValue(item.FII_Put_Change),
