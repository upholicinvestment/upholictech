...
class DhanClient:
    def __init__(self):
        ...
        self.collection = self.db["bse_equity"]
        self.total_stocks = 0
    ...
    def fetch_bse_equity_instruments(self):
        ...
        try:
            ...
            if bse_df.empty:
                print("No BSE equity instruments found.")
                return None
            ...
            return bse_df[available_columns].rename(columns=columns_map)

        except Exception as e:
            print(f"Error fetching BSE instrument list: {str(e)}")
            return None

    def count_bse_equity_stocks(self):
        df = self.fetch_bse_equity_instruments()
        if df is not None and not df.empty:
            self.total_stocks = len(df)
            print(f"\n✅ Total BSE Equity stocks: {self.total_stocks}\n")
            return self.total_stocks
        else:
            print("\n❌ No BSE Equity stocks found.\n")
            return 0

    def check_all_stocks_stored(self):
        if len(self.stored_stocks) >= self.total_stocks:
            print("\n🎉 All BSE stock data has been successfully stored in the database!\n")
            return True
        return False

    def run_data_collection_cycle(self):
        instruments_df = self.fetch_bse_equity_instruments()
        version = "v2"

        if instruments_df is None or instruments_df.empty:
            print("⚠️ Using fallback instruments (RELIANCE, HDFCBANK)")
            instruments = [
                (marketfeed.BSE, "3045", marketfeed.Full),
                (marketfeed.BSE, "1394", marketfeed.Full)
            ]
            self.total_stocks = 2
        else:
            instruments = []
            for _, row in instruments_df.iterrows():
                ...
                instruments.append((marketfeed.BSE, security_id, marketfeed.Full))
